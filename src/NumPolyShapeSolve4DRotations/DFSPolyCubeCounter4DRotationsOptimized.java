package NumPolyShapeSolve4DRotations;
...
//Making a problem that counts shapes where the reflection of a 3D shape is considered the same as the 3D shape
// Reflections in 3D are rotations in 4D:
// Tibees video going over this: https://www.youtube.com/watch?v=jH_QIBtX1gY

public class DFSPolyCubeCounter4DRotationsOptimized {


	public static boolean debugPrintEvery5Seconds = false;

	//I doubled the amount of rotations because I'm allowing reflections:
	public static final int NUM_ROTATIONS_3D = 48;
	...
	public static void generateAllTheNudges() {
		...
		for(int i=0; i<NUM_NEIGHBOURS_3D; i++) {
			for(int j=0; j<NUM_ROTATIONS_2D; j++) {
				...
				//Step 7: Make the chiral version of the current set of nudges:
				// I just switched the last 2 columns (i.e. the columns that poke into the 3rd dimension for the nugdeBasedOnRotation array.)
				int arrayToFillChiral[][] = nugdeBasedOnRotationAllStartingSymmetries[NUM_ROTATIONS_3D/2 + NUM_ROTATIONS_2D * i + j];
				
				for(int i2=0; i2<arrayToFillChiral.length; i2++) {
					for(int j2=0; j2<arrayToFillChiral[0].length; j2++) {
						if(j2 == 4) {
							arrayToFillChiral[i2][j2] = arrayToFill[i2][j2 + 1];
						} else if(j2 == 5) {
							arrayToFillChiral[i2][j2] = arrayToFill[i2][j2 - 1];
						} else {
							arrayToFillChiral[i2][j2] = arrayToFill[i2][j2];
						}
					}
				}
			}
	...
	public static void main(String args[]) {
		...
		//This matches this OIES sequence up to at least N=15: https://oeis.org/A038119
		...
		//For N=15: "Final number of unique solutions: 3934285874" (took 13 hours...)
